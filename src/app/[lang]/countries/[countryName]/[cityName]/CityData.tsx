...
"use client"
import { getCityData } from "@/lib/helper";
import { useEffect, useState } from "react";
import { useParams } from "next/navigation";
import { PrayerTimes, CalculationMethod, Madhab } from "adhan"; // or your package

const Page = () => {
  const params = useParams();
  const { country, city } = params as { country: string; city: string };
  ...
  useEffect(() => {
    const result = getCityData(country, city);

    if (!result) {
      console.error("Invalid city or country");
      return;
    }

    const { country: countryData, city: cityData } = result;

    // Filter out current city to get other cities
    const others = countryData.cities.filter((c) => c.name !== cityData.name);
    setOtherCities(others);

    const date = new Date();
    const dateInCity = new Date(date.toLocaleString("en-US", { timeZone: cityData.timezone }));
    ...
    setPrayerTimes({
      fajr: formatTime(prayer.fajr),
      dhuhr: formatTime(prayer.dhuhr),
      asr: formatTime(prayer.asr),
      maghrib: formatTime(prayer.maghrib),
      isha: formatTime(prayer.isha),
    });
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      const dInCity = new Date(d.toLocaleString("en-US", { timeZone: cityData.timezone }));
  ...
  return (
    <div className="p-4">
      ...
      {monthlyTimes.length > 0 && (
        <div className="mt-4">
          <h2>Monthly Prayer Times</h2>
          <ul>
            {monthlyTimes.map((day, i) => (
              <li key={i}>
                {day.date} - Fajr: {day.fajr}, Dhuhr: {day.dhuhr}, Asr: {day.asr}, Maghrib: {day.maghrib}, Isha: {day.isha}
              </li>
            ))}
          </ul>
        </div>
      ...
      {otherCities.length > 0 && (
        <div className="mt-4">
          <h3>Other cities in this country:</h3>
          <ul>
            {otherCities.map((c, i) => (
              <li key={i}>
